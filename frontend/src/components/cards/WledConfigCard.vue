<template>
  <div class="card wled-config-card">
    <div class="card-header">
      <svg viewBox="0 0 24 24" fill="currentColor" class="header-icon"><path :d="mdiLedStripVariant" /></svg>
      {{ card.title || 'WLED Devices' }}
    </div>
...
function openAdd() {
  dialog.value = { name: '', id: '', ip: '', type: 'RGBW', isNew: true, error: '' };
}

function openEdit(d) {
  dialog.value = { ...d, type: d.type ?? 'RGBW', isNew: false, error: '' };
}
