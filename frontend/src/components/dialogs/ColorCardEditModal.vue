<template>
  <Teleport to="body">
    <div class="modal-backdrop" @click.self="$emit('cancel')">
      <div class="modal">
        <h2 class="modal-title">Edit Color Card</h2>
        ...
        <div class="item-list">
          ...
          >
            ...
            <span class="item-count">{{ presetCountFor(cat.name) }}</span>
          </button>
...
fetch('/api/colors')
  ...
  .then(data => {
    ...
    localPresets.value = data.presets ?? [];
  })
  .catch(() => {});

function presetCountFor(name) {
  return localPresets.value.filter(p => p.category === name).length;
}
...
function removeCategory() {
  ...
  localPresets.value = localPresets.value.filter(p => p.category !== name);
...
async function save() {
  ...
  try {
    const res = await fetch('/api/colors', {
      ...
      body: JSON.stringify({ categories: localCategories.value, presets: localPresets.value }),
    });
